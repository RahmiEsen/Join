...
export class BoardComponent implements OnInit {
  ...
  onListTitleChanged(event: { listId: string; newTitle: string }): void {
    this.taskService.updateTaskList(event.listId, event.newTitle).subscribe({
      next: (updatedList) => {
        const listInComponent = this.taskLists.find(
          (l) => l.id === event.listId
        );
        if (listInComponent) {
  ...
  dropList(event: CdkDragDrop<TaskList[]>) {
    ...
    this.boardService.updateTaskListOrder(orderedIds).subscribe({
      next: () => {
        console.log(
          'Reihenfolge der Listen erfolgreich im Backend gespeichert.'
        );
      },
      error: (err) => {
  ...
  dropTask(event: CdkDragDrop<Task[]>) {
    ...
    this.taskService.updateTask(movedTask.id, updatePayload).subscribe({
      ...
      error: (err) =>
        console.error('Fehler beim Speichern der Task-Position:', err),
    });
  }
  ...
  handleMoveList(event: { listId: string; newPosition: number }): void {
    const originalIndex = this.taskLists.findIndex(
    ...
    this.boardService.updateTaskListOrder(orderedIds).subscribe({
      next: () => {
        console.log(
          'Reihenfolge der Listen erfolgreich durch Menü-Aktion gespeichert.'
        );
      },
      error: (err) => {
        console.error('Fehler beim Speichern der neuen Reihenfolge:', err);
  ...
  handleMoveTask(event: {
    ...
    const targetList = this.taskLists.find(
      (list) => list.id === event.targetListId
    );
    if (targetList) {
    ...
    this.taskService.updateTask(taskToMove.id, updatePayload).subscribe({
      next: () =>
        console.log(`Task-Position erfolgreich via Menü gespeichert!`),
      error: (err) =>
        console.error('Fehler beim Speichern der Task-Position:', err),
    });
  }
  ...
  onDeleteTask(taskId: string): void {
    this.taskService.deleteTask(taskId).subscribe({
      ...
      error: (err) => {
        console.error(`Fehler beim Löschen des Tasks mit ID ${taskId}:`, err);
      },
    });
  }
}
